...
// --- Dashboard & Search ---
let allCustomers = []; // Store for search
...
window.loadDashboard = async () => {
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            allCustomers.push({ id, ...data, remaining });
        });

        totalDueEl.innerText = totalDue.toLocaleString();
        totalCustEl.innerText = allCustomers.length;
...
window.filterCustomers = () => {
    ...
    const filtered = allCustomers.filter(c => 
        ...
        (c.customId && c.customId.toString().includes(query))
    );
...
function renderTable(customers) {
    ...
    customers.forEach(c => {
        ...
        row.innerHTML = `
            <td class="p-4 text-purple-400 font-bold">#${c.customId || '---'}</td>
            <td class="p-4 flex items-center">
                <img src="${c.imageUrl || 'https://via.placeholder.com/50'}" class="w-10 h-10 rounded-full object-cover mr-3 border border-purple-500">
                <div>
                    <span class="block font-medium text-white">${c.name}</span>
                </div>
            </td>
            <td class="p-4 hidden md:table-cell text-gray-300">${c.phone}</td>
            <td class="p-4 font-bold ${c.remaining > 0 ? 'text-red-400' : 'text-green-400'}">
                ${c.remaining.toLocaleString()}
            </td>
            <td class="p-4">
                <a href="customer-profile.html?id=${c.id}" class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-sm transition">View</a>
            </td>
        `;
...
window.submitCustomer = async (e) => {
    ...
    btn.disabled = true;
    btn.innerText = "Uploading Images & Saving...";
    ...
    try {
        ...
        if(photoFile) {
            const ref1 = ref(storage, `customers/${customId}_photo_${Date.now()}`);
            await uploadBytes(ref1, photoFile);
            imageUrl = await getDownloadURL(ref1);
        }

        if(cnicFile) {
            const ref2 = ref(storage, `customers/${customId}_cnic_${Date.now()}`);
        ...
        await addDoc(collection(db, "customers"), {
            customId, name, phone, address, imageUrl, cnicUrl,
        ...
        btn.disabled = false;
        btn.innerText = "Save Customer Record";
    }
};
...
window.loadProfile = async () => {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            document.getElementById('p-cnic-img').src = data.cnicUrl || "https://via.placeholder.com/300x200?text=No+CNIC";
